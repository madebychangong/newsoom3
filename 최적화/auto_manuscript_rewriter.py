...
## 규칙 4: 나머지 부분 키워드 ⚠️ 정확히 맞추기 (초과 금지!)
**⚠️ 중요: 아래 숫자는 "최소값"이 아니라 "정확한 값"입니다!**
**더 많이 넣으면 안 됩니다!**

**D열 - 나머지 부분 통키워드 (첫 문단 제외):**
{target_whole_str}
→ 이 값보다 적으면 ❌, 많으면 ❌, 정확히 같으면 ✅

**E열 - 나머지 부분 조각키워드 (첫 문단 제외):**
{target_pieces_str}
→ 각 조각키워드마다 정확히 이 횟수만큼만!
...
1. ✅ 첫 문단에 "{keyword}" 정확히 2번? (조사 붙은 건 카운팅 X)

2. ✅ 전체 원고에서 "{keyword}"로 시작하는 줄 정확히 2개? (조사 붙은 건 카운팅 X)

3. ✅ 나머지 부분(첫 문단 제외)에서:
   - D열 값 정확히 맞음? (초과 금지!)
   - E열 값 정확히 맞음? (초과 금지!)

4. ✅ "{keyword}에", "{keyword}를" 같은 표현 없나?

5. ✅ 글자수 300~900자 범위 내? (공백/줄바꿈 제외)

**⚠️ 특히 중요: 키워드 개수를 더 많이 넣으면 안 됩니다!**
**정확히 목표값만큼만 넣으세요!**

위 5가지 중 하나라도 실패면 다시 작성하세요!
