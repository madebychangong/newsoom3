...
# 현재 상태 분석
- 글자수: {analysis['chars']}자 (목표: 300~900자) {'✅' if analysis['chars_in_range'] else '❌'}
- 첫 문단 통키워드: {analysis['첫문단_통키워드']}회 (목표: 2회) {'✅' if analysis['첫문단_통키워드'] == 2 else '❌'}
- 통키워드로 시작하는 문장: {analysis['통키워드_문장시작']}개 (목표: 2개) {'✅' if analysis['통키워드_문장시작'] == 2 else '❌'}

## 나머지 부분 통키워드 목표
{target_whole_str}
현재 상태: {analysis['나머지_통키워드']}

## 나머지 부분 조각키워드 목표
{target_pieces_str}
현재 상태: {analysis['나머지_조각키워드']}

## 서브키워드 (2회 이상 등장하는 단어)
목표: {analysis['subkeywords']['target']}개
현재: {analysis['subkeywords']['actual']}개
...
- ✅ "{keyword} 찾아서" → 카운팅 O
- ✅ "{keyword}." → 카운팅 O
- **한 글자 조사는 억지로 띄어쓰기 하지 말고, 문장을 우회해서 작성**
  - ❌ "{keyword} 을" (부자연스러움)
  - ✅ "{keyword} 관련해서" (자연스러움)
...
- 너무 정제하지 말 것
- 사람이 쓴 느낌 유지
- 댓글 유도 문장 포함 (예: "정보 공유 좀 해주세요")

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

# 출력
**수정된 원고만** 출력하세요. 설명이나 주석 없이 원고만.
"""
        return prompt
