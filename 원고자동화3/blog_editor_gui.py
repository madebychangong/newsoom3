...
- **서브 키워드 목록 수**: {extra_keyword_count}개
  ...
  → **중요**: 단어가 부족하면 중복 문장부호 적극 활용 (^^, ??, !!, ~~, .., ㅠㅠ, ㅜㅜ, ㅎㅎ 등)
...
- **서브키워드 부족**: 중복 문장부호를 적극 활용하여 채우기
  → ^^, ??, !!, ~~, ..., ㅠㅠ, ㅜㅜ, ㅎㅎ 등을 문장 끝이나 중간에 자연스럽게 배치
...
## 7. 글자수 (매우 중요!)
- **반드시 {target_chars}자에 최대한 가깝게 작성** (허용 범위: {target_chars - char_tolerance}~{target_chars + char_tolerance}자)
- 글자수가 부족하면 자연스러운 설명 추가, 많으면 불필요한 부분 제거
- **글자수는 다른 규칙만큼 중요하므로 반드시 범위 내로 맞출 것**
...
**특히 중요:**
1. **글자수 {target_chars - char_tolerance}~{target_chars + char_tolerance}자 범위 내로 반드시 맞추기** (다른 규칙과 동등하게 중요!)
2. **첫 문단은 무조건 4문장 이상**으로 작성하고, '{row_data['keyword']}' 정확히 2회 포함
3. **첫 문단 이후 나머지 문단에서** 통키워드와 조각키워드는 지정된 횟수만큼만 사용
4. 서브키워드 목표 개수를 맞추기 위해 중복 문장부호(^^, ??, !!, ㅠㅠ, ㅜㅜ, ..., ~~ 등) 적극 활용
