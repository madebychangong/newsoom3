...
## 7. 글자수 (최우선 규칙!)
- **절대적 필수 조건: {target_chars - char_tolerance}자 ~ {target_chars + char_tolerance}자 범위 내**
- 목표 글자수: {target_chars}자
- 이 범위를 벗어나면 절대 안 됨 (키워드 규칙보다 우선!)
- 글자수 초과 시: 불필요한 형용사, 부사, 중복 표현 삭제
- 글자수 부족 시: 구체적인 예시, 부연 설명, 경험담 추가
- **작성 후 반드시 글자수를 세어 범위 내인지 확인할 것**
...
1. 먼저 자연스러운 글 작성
2. 작성 후 글자수 확인
3. **{target_chars - char_tolerance}자 미만이면 내용 추가, {target_chars + char_tolerance}자 초과면 내용 삭제**
4. 범위 내로 맞춘 후 최종 제출

**절대 규칙:**
1. **[최우선] 글자수 {target_chars - char_tolerance}~{target_chars + char_tolerance}자 (이 범위를 벗어나면 실패!)**
2. **첫 문단은 무조건 4문장 이상**으로 작성하고, '{row_data['keyword']}' 정확히 2회 포함
3. **첫 문단 이후 나머지 문단에서** 통키워드와 조각키워드는 지정된 횟수만큼만 사용
4. 서브키워드 목표 개수를 맞추기 위해 중복 문장부호(^^, ??, !!, ㅠㅠ, TT, ..., ~~ 등) 적극 활용
5. 통키워드로 시작하는 문장 2~3개 포함
6. **금칙어는 절대 사용하지 말고 문맥에 맞는 대체어 사용**
7. **2~4문장마다 빈 줄로 문단을 구분하여 가독성 높이기** (줄바꿈 2번 = 빈 줄 1개)

**경고: 글자수가 {target_chars - char_tolerance}자 미만 또는 {target_chars + char_tolerance}자 초과하면 규칙 위반!**
