...
## 7. 글자수 (절대 필수!)
- **필수 조건: {target_chars - char_tolerance}자 ~ {target_chars + char_tolerance}자 범위 내**
...
**작성 순서 (반드시 지킬 것):**
1. 먼저 모든 규칙을 고려하여 자연스러운 글 작성
2. 작성 후 글자수 확인
3. **{target_chars - char_tolerance}자 미만이면 내용 추가, {target_chars + char_tolerance}자 초과면 내용 삭제**
4. 모든 규칙이 충족되었는지 최종 확인 후 제출

**절대 필수 규칙 (모두 동등하게 중요! 하나라도 어기면 안 됨!):**
1. **글자수: {target_chars - char_tolerance}~{target_chars + char_tolerance}자 범위 내 (엄수!)**
2. **첫 문단: 무조건 4문장 이상 + '{row_data['keyword']}' 정확히 2회 포함 (엄수!)**
3. **통키워드/조각키워드: 첫 문단 이후 나머지 문단에서 지정 횟수만큼 반복 (엄수!)**
4. **서브키워드: 목표 개수 맞추기 (중복 문장부호 ^^, ??, !!, ㅠㅠ, TT, ..., ~~ 적극 활용) (엄수!)**
5. **통키워드로 시작하는 문장: 지정된 개수만큼 포함 (엄수!)**
6. **금칙어: 절대 사용 금지, 무조건 대체어 사용 (엄수!)**
7. **문단 구분: 2~4문장마다 빈 줄로 문단 나누기 (줄바꿈 2번) (엄수!)**

**경고: 위 규칙 중 단 하나라도 어기면 전체 실패! 모든 규칙을 100% 준수해야 함!**
